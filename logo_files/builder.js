...
GS.Editor = function (options) {
    ...
    // Onboarding instance
    this.onboarding = new GS.Editor.Onboarding(this.sidebar.config, this.helpers, this.core);
...
GS.Editor.prototype.mouseDownEvent = function ($item, $itemPart, initialEvent, itemPosition, itemDimensions) {
    ...
    // If the item is not selected before, select it ...
    if (itemAlreadyActive === false) {
    ...
    $(this.config.el.editor).on('mouseup', function () {
        if (itemAlreadyActive === true) {
            if (isMoving === false) {
                // If the part was already selected before, deselect it
                if ($itemPart[0] === this.helpers.getItemActivePart($item)[0]) {
                    this.deselectItemParts($item);
                    this.selectItem($item);

                    $(this.config.el.editor).trigger(GS.Editor.Events.Item.Part.Deselected, [
                        {
                            item: $item,
                            part: $itemPart,
                            type: itemType,
...
/**
 * Removing the overlay loading class when we're ready to show the editor
 * @return {Object} Editor instance
 */
GS.Editor.prototype.resetOverlay = function () {
    $(this.config.el.overlay).removeClass(this.config.coreClasses.overlayLoading);
...
GS.Editor.Core.prototype.getSvgCode = function (url, ev) {
    ...
    $.ajax({
        ...
        success: function (str) {
            ...
            var svgCode = $(xmlSerializer.serializeToString($($.parseXML(str)).find('svg')[0]))[0];
            var viewBox = svgCode.getAttribute('viewBox').split(' ') || [0, 0, 0, 0];
            ...
            try {
                transformedNodes = svgCode.querySelectorAll('[transform]');
            ...
            var fullCode = svgCode.innerHTML;
...
GS.Editor.Sidebar.prototype.registerEvents = function () {
    ...
    $(this.config.el.sidebar).on('click', this.config.el.sidebarActionTriggerExpand, function (ev) {
        ...
        if ($(this).hasClass(self.helpers.getRawName(self.config.el.sidebarActions.editor))) {
            ...
            if ($(this).hasClass(self.helpers.getRawName(self.config.el.sidebarActions.about))) {
                self.prepareAbout();
            }
        }
...
GS.Editor.Sidebar.prototype.doCalculateMenus = function () {
    this.doCalculateItemsSidebarMenuHeaders();
    ...
    $(this.config.el.sidebarMenus.allItems).each(function (index, menu) {
        ...
        if ($menu.hasClass(this.helpers.getRawName(this.config.el.sidebarMenus.images)) ||
            ...
            menuContentHeight = this.windowHeight - this.itemsSidebarMenuHeadersHeight;
        } else {
...
/**
 * Calculating the headers (gfx+title) of the items sidebar menus
 * so we can later calculate the expansion of images and shapes menus
 * (as they need to cover the page)
 *
 * @return {Number} Sum of items sub-sidebar's height
 */
GS.Editor.Sidebar.prototype.doCalculateItemsSidebarMenuHeaders = function () {
    $(this.config.el.sidebarMenus.allItemsTitles).each(function (index, menuHeader) {
        var $header = $(menuHeader);

        if ($header.is(':visible')) {
            this.itemsSidebarMenuHeadersHeight += $(menuHeader).outerHeight();
        }
    }.bind(this));

    return this.itemsSidebarMenuHeadersHeight;
};
...
GS.Editor.Sidebar.prototype.addNewTagline = function () {
    ...
    var $tagline = $([
        ...
        '    <input data-target-tagline="" class="menu--identity__input menu--identity__input--tagline" type="text" placeholder="Your tagline">',
...
GS.Editor.Sidebar.prototype.resetAboutTemplate = function() {
    $(this.config.el.about).removeClass(this.config.coreClasses.aboutActive);
};

GS.Editor.Sidebar.prototype.prepareAbout = function() {
    setTimeout(function() {
        $(this.config.el.about).addClass(this.config.coreClasses.aboutActive);
    }.bind(this), 0);
};
;/* -----------------------------------------
...
GS.Editor.Toolbars.prototype.updateMatchingFills = function(toolbarType, $toolbarParent, hex) {
    ...
    var setMatchingFills = function() {
        for (var i = 0; i < matchingColours.length; i++) {
            var targetEl = $toolbarParent.find(this.config.el.toolbarComponents.fill.matchItem)[i];
...
GS.Editor.Toolbars.ColourHistory.prototype.generateColourHistory = function() {
    var $colourHistoryWrapper = $(this.config.el.colourHistoryWrapper);
    var finalHtml = '';

    if ($colourHistoryWrapper.length === 0) {
        return;
    }

    for (var i = this.colours.length - 1; i >= 0; i--) {
        finalHtml = finalHtml + this.colourHistoryItemHtml.replace(/{{{colour}}}/g, this.colours[i]);
    }
...
GS.Editor.Toolbars.Shape.prototype.generateGradientElement = function() {
    if (this.helpers.hasItemPartActive()) {
        ...
        if (fill.indexOf('url(') == -1) {
            ...
            this.colourpickerGradientFirst.setHex('#666666');
            this.colourpickerGradientSecond.setHex('#666666');
...
GS.Editor.Toolbars.Image.prototype.prepareImageToolbar = function(toolbarType) {
    ...
    // Registering click event for clicking on Colour Matching/History circles
    $(this.config.el.colourpicker.image.toolbarParent).on('click', '.toolbar-component__fill--solid [data-' + this.config.attributes.toolbarChangeFillTrigger + ']', function() {
        var $this = $(this);
        var targetHex = $this.attr('data-' + self.config.attributes.toolbarChangeFillTrigger);
    ...
    $(this.config.el.colourpicker.image.toolbarParent).on('click', '.toolbar-component__fill--gradient [data-' + this.config.attributes.toolbarChangeFillTrigger + ']', function() {
        var $this = $(this);
        var targetHex = $this.attr('data-' + self.config.attributes.toolbarChangeFillTrigger);
...
GS.Editor.Importer.prototype.registerEvents = function() {
    ...
    $(this.config.el.triggers.importerBusiness).on('keydown', function(ev) {
        if (ev.keyCode == self.helpers.keys.KEYENTER) {
            ...
            function updateBusinessText() {
                ...
                $(self.config.el.business.text).html(escaped);
    ...
    $body.on('keydown', this.config.el.triggers.importerTagline, function(ev) {
        var $this = $(this);
        var targetTagline = $this.attr('data-target-tagline');

        if (ev.keyCode == self.helpers.keys.KEYENTER) {
            $('#' + targetTagline).find('text').text($this.val());
...
OnboardingStage.prototype.event = function (event) {
    if (!event) {
        return this._event;
    }
...
OnboardingStage.prototype.target = function (target) {
    if (!target) {
        return this._target;
    }
...
OnboardingStage.prototype.position = function (top, left) {
    if (!top || !left) {
 ...
 * @param {Object} core      Editor core
 */
GS.Editor.Onboarding = function (options, helpers, core) {
    this.options = options || {};
    ...
    this.mobileStages = [
        (new OnboardingStage(GS.Editor.Events.GA.MobileStage1))
            .target('.menu--identity')
            .action(function () {
                var $businessInput = $('.menu--identity__input--business');

                if ($businessInput.val() != '') {
                    $body
                        .removeClass(self.config.coreClasses.onboardingStageOne)
                        .addClass(self.config.coreClasses.onboardingStageTwo);

                    $('.menu--images').trigger('click');

                    if ($('.logo-listing--mobile--active').length == 0) {
                        setTimeout(function () {
        ...
        (new OnboardingStage(GS.Editor.Events.GA.MobileStage2))
            .target('.menu--images')
            .action(function () {
                $('.' + self.config.coreClasses.onboardingGuideActive)
                    .removeClass(self.config.coreClasses.onboardingGuideActive);

                $body.removeClass(self.config.coreClasses.onboardingStageTwo);
    ...
    this.desktopStages = [
            ...
            .action(function () {
                ...
                if ($businessInput.val() != '') {
                    $body
                        ...
                        .addClass(this.config.coreClasses.onboardingStageTwo);
                    $('.menu--images').trigger('click');
                } else {
                    $businessInput.focus();
                }
        ...
        (new OnboardingStage(GS.Editor.Events.GA.Stage2))
            ...
            .action(function () {
                $body
                    .removeClass(this.config.coreClasses.onboardingStageTwo)
                    .addClass(this.config.coreClasses.onboardingStageThree);
                if ($('.categories__category--active').length === 0) {
                    $('[data-category-id="0"]').trigger('click');
                }
            }),
        (new OnboardingStage(GS.Editor.Events.GA.Stage3))
            .position(190, this.helpers.isDevice('tablet', 'portrait') ? 230 : 480)
    ];
...
GS.Editor.Onboarding.prototype.registerEvents = function () {
    $('.' + this.config.coreClasses.onboardingActive + ' .menu--images').one('click', function () {
        if (this.stageIndex !== 1) {
            this.doStage(1);
        }
    }.bind(this));

    $('.' + this.config.coreClasses.onboardingActive).on('click', '.categories__category', function () {
...
/**
 * Storing the stage data to our main stage array
 *
 * @return {Object} Onboarding instance
 */
GS.Editor.Onboarding.prototype.doSetupStages = function () {
    if (this.helpers.isDevice('phone')) {
...
GS.Editor.Onboarding.prototype.doStage = function (stageIndex) {
    ...
    var og = this.helpers.isDevice('phone') ? this.config.el.mobileOnboardingGuide : this.config.el.onboardingGuide;

    if (stage.event()) {
        this.helpers.submitGaEvent(stage.event());
    }

    $('.' + this.config.coreClasses.onboardingGuideActive).removeClass(this.config.coreClasses.onboardingGuideActive);

    // Position the guide
    var $guide = $(og.stageSelector + stageIndex);
    ...
    $guide.addClass(this.config.coreClasses.onboardingGuideActive);

    // Initiate the stage
    if (stage.target()) {
        $(stage.target()).trigger('click');
    }

    $body.on('click', og.button, function (ev) {
        ev.preventDefault();

        if (typeof stage.action() == 'function') {
            stage.action().call(this);
        } else {
            this.doStage(stageIndex + 1);
        }
    }.bind(this));
};
...
GS.Editor.Onboarding.prototype.doStart = function () {
    ...
    setTimeout(function () {
        ...
        setTimeout(function () {
            $('.menu--identity__input--business').focus();
        }.bind(this), 500);
    }.bind(this), 300);
};
...
GS.Editor.Onboarding.prototype.doEnd = function () {
    ...
    $('.' + this.config.coreClasses.onboardingActive).one('click', '.logo-listing__logo,.logo-listing--mobile__logo', function () {
        $body
    ...
    setTimeout(function () {
        ...
        $body.removeClass(this.config.coreClasses.onboardingStageThree);
        $(this.config.el.onboardingGuide.main).removeClass(this.config.coreClasses.onboardingGuideActive);
